...
void THCudaTensor_getri(THCState *state, THCudaTensor *ra_, THCudaTensor *a)
{
  ...
  THCnmemCheck(THCudaMalloc(state, (void**)&d_matrices1, matrices_size));
  THCnmemCheck(THCudaMalloc(state, (void**)&d_matrices1_const, matrices_size));
  THCnmemCheck(THCudaMalloc(state, (void**)&d_matrices2, matrices_size));
  ...
  int info;
  int *info_gpu;
  THCnmemCheck(THCudaMalloc(state, (void**)&info_gpu, sizeof(int)));

  int *ipiv_gpu;
  THCnmemCheck(THCudaMalloc(state, (void**)&ipiv_gpu, n * sizeof(int)));
  ...
  THCnmemCheck(THCudaFree(state, ipiv_gpu));
  THCnmemCheck(THCudaFree(state, info_gpu));
